...
The thesis-level recommendation — declaring which edge families are canonical and which yield to prose — would require a new section in `scope/arc.md`. The current text states "the graph is the canon database" without qualification (line 9). The Wikidata 2018 RfC outcome is the published precedent that this unqualified claim does not survive 10k-scale prose authoring. Adopting the qualification is the proposed change; rejecting the precedent and keeping the claim unqualified is the alternative. This document does not adjudicate; it surfaces the choice.
