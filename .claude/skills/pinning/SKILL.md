...
data/pinned.json               (PinnedData: { [id]: { coordinates, zoomLevel, type, pinnedAt } })
   │
   │ scripts/finalize-locations.ts  (merges work-queue + pinned + descriptions → final shape)
   ▼
...
- **`/api/pin` writes the filesystem.** It uses `fs.writeFileSync` against `data/work-queue.json` and `data/pinned.json`. This is by design — pinning is local-only and doesn't deploy to Vercel. Do not refactor it to a database or to read-only without a plan for preserving the existing files.
...
- UI: `src/app/pin/page.tsx`, `src/components/PinningMap.tsx`
- API: `src/app/api/pin/route.ts` (GET current, POST pin/skip, PUT jump/back)
- Types: `src/types/pinning.ts`
...
- Data: `data/work-queue.json` (queue + status), `data/pinned.json` (id → pin), `data/locations.json` (final, public-facing)
