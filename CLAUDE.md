...
- **Entity files are canonical**: never hand-edit the 5 generated outputs — `data/locations.json`, `public/locations.json`, `public/codex-edges.json`, `data/codex/compiled.json`, `public/codex-search.json` — edit the entity file in `content/codex/entities/` and re-run the generator. The build does unguarded `writeFileSync` to all 5 with no lockfile, so never run two `build-codex.mts` concurrently (parallel writers are fine; serialize the build).
