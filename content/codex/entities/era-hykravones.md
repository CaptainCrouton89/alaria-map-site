...
The daemon-survival passage uses the RE-EMBODIMENT model (user ruling 2026-06-01; full derivation in context/decision-reembodiment-model.md and the canon-check context/explore-daemon-reembodiment.md; governed by context/decision-claim2-daemon-races.md). It supersedes the earlier "bound into a song via Deoric" framing, which the user rejected as incoherent. The chain, grounded in the death-cosmology (plane-life-and-death.md, systems/first-death.md, systems/celestia-return.md):
1. WHAT A DAEMON IS: a spirit (the self, made of Ezz) persisting in Celestia, sustained by worship/remembrance. Soul (Aurus) and shadow (Nydus) are borrowed counterbalances bound to a spirit by astral/malstaric threads; they CYCLE (reused for new lives). Only the spirit is permanent and ends when forgotten.
2. WHY THEY DIED: kill the worshippers → prayer stops → the name goes quiet → forgotten → the spirit ends. Daemons died WITH their people.
3. THE SURVIVAL MOVE = re-embodiment via the INCARNATION RITE (user-chosen, with the dark refinement): a daemon cannot return to its own long-dissolved body (celestia-return: return needs a preserved anchor). Instead its faithful work a rite that UNWINDS A LIVING PERSON'S SPIRIT and snuffs it out (the true death; the self annihilated), leaving the host's body + soul + shadow intact, then draws the daemon's spirit down from Celestia to take the vacant place. Morally monstrous: done WILLINGLY (a devotee's last gift) or UNWILLINGLY (a victim seized). A person is ended so a daemon may live. This establishes NEW canon (the displacement rite), now noted in plane-life-and-death.md, celestia-return.md, plane-celestia.md.
4. THE PRICE = fully mortal (user-chosen): seated in flesh the daemon is no longer a daemon — no worship-reserve, no miracles, no boundary-reach. It ages and dies as a mortal; its spirit then fades to Celestia like anyone's. No Deoric self-transformation involved ("we don't need deoric").
5. RACE-FOUNDING: a re-embodied daemon can breed; its line becomes a "daemon-descended people" — descent by ancestry from a god-turned-mortal, NOT a spirit passing blood (no paradox; the user chose fully-mortal/ancestry over a marked bloodline). No race named here; which peoples and what (if anything) marks them stays the parallel racial-origins effort's job (session d5c13b10).
6. AELWENNAR / ELUVARIN = a SEPARATE survival path (agent-006 verdict): they rooted their spirit into a lasting elven form (Long-Song / First Grove) and STAYED daemons — did not take flesh, displaced no one. Their entities are correct as-is and untouched; the era frames them as the rarer "wore a form, stayed a daemon" road. The three peoples in the following paragraph descend from Gaea's fallen children — a separate population, kept distinct.
