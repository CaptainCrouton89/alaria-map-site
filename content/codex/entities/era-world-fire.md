...
id: "era-world-fire"
name: "The World Fire"
blurb: "10 BSD, calendar zero. A lone fire-paragon over-channeled the Yolus seam until it ruptured, ending the BSD reckoning and opening the SD age."
entityType: era
tags: ["history", "era"]
weight: standard
atmosphere: dangerous
relations:
  - { rel: history, kind: within, target: "era-great-expansion" }
sources:
...
<!-- author-notes -->
Cause is asserted canon per the Fire-slice sub-plan (decision #1, design §5 lever): a lone deep-history fire-paragon over-channeled the Yolus seam beyond their own emotional capacity, fuel-demand outran the self, the seam ruptured, fire spilled out. NOT a Firemage Corps cabal (the Corps is ~2776 SD, ~2,786 years later — anachronism). Deliberately withheld: only the EPICENTER LOCATION and the specific working, never the cause.

Epicenter coordinates are deliberately NOT fixed — this slice mints no new geography or pin. No location is canon yet. A later geography pass may place the burned glass-scar; if so it must stay consistent with (a) the erratic post-rupture Yolus seam and (b) the located Cinderbound. The "mysteries must have answers" rule is satisfied by the in-world reason the location is lost (the knowers died in it, the records burned), which is a ruin-legacy gap, not a cause-hedge.

§9 ruin-legacy elements used: (1) terrain scar = the unnamed/unlocated glassed-and-ash burned country (the inset); (2) leyline instability = the erratic Yolus seam, stated present-tense as the still-active legacy; (3) spawned ecology = the Cinderbound (creature-cinderbound, authored by the BEINGS slice), named in two sentences as the located living remnant, detail deferred to their own file; (4) knowledge gap = the lost epicenter; (5) sealed artifact = omitted (added nothing, would risk new geography).

Cinderbound resolve the epicenter paradox: people WERE caught (they became the Cinderbound); only the coordinates + mechanism-knowledge are lost. "Something other than dead" gestures at the §8 element-bound register without authoring the mechanism here.

No edges authored on this file (one direction only): Fernling's `participatedIn → era-world-fire` lives on person-fernling-whist; magic-fire's `sourceOf → era-world-fire` lives on magic-fire. Frontmatter id/entityType/weight/atmosphere and the `within → era-great-expansion` edge are untouched. Blurb rewritten to assert the cause and drop the banned "civilizational threshold" puffery.
