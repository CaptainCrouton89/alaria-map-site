---
id: "creature-hourbound"
name: "Hourbound"
entityType: creature
blurb: "People whose living slipped its moment when Izzus fused into them: animate but not living, desynced from the clock everything else keeps."
tags: ["creature", "time", "haunting"]
weight: standard
atmosphere: cursed
relations:
  - { rel: cosmology, kind: attunedTo, target: "magic-time", note: "Izzus fused into the strand-set; the life-rate drifted off the world's clock" }
review:
  aiWritten: false
  action: keep
---
...
> We had a counter named Ressa who could hold a stretch longer than any of us. One season she stopped coming all the way back out of it. She still works the floor. She is always early to the bell and never once surprised, and not one of us will sit at her table.
> — a Besnoumeru time-gang factor
...
<!-- author-notes -->
GM answers (mysteries-must-have-answers):

WHAT THEY WANT — beneath the drift toward a "then" (Izzus register: patience/foreboding/regret/anticipation, feeling oriented away from the present, per magic-time:19), the load-bearing want is to CATCH THEIR OWN MOMENT — to be back in step with the world's clock. They cannot aim themselves back: the element's signature failure is drift (magic-time:21), so a life knocked off its moment cannot steer back onto it any more than a shaper can hold a pocket dead on its mark. Not malicious; stuck. This deliberately parallels the Cinderbound want ("finish dying") in a Time key: the Cinderbound's death-road is welded shut, the Hourbound's runs at the wrong rate.

CAN THEY BE LAID TO REST — YES, and more readily than the Cinderbound, by TWO distinct routes:
(1) WAITING. A late/early road is slow, not broken; a slow road still ends. Given enough time the remnant catches its moment and departs on its own. This is the Time-key answer and the main one.
(2) For STALLED and REPEATING roads (no forward motion left), the only rest is a VOID/NILUS working to un-make the binding directly, freeing the thread to take whatever road remains. A TIME-shaper is explicitly useless: re-timing a drifted soul = matching it to the Great Cycle, which is past every mortal hand (locked limit, magic-time:17). The Salt Tomb (id 3816) is the prose-only worst-case image of a total stall — I assert NOTHING about its sealed contents (salt-tomb.md keeps that unknown); the watchers recognize the SHAPE of a stall without claiming to know what is inside.

§8 PROOF (death-by-degree axis, made explicit in "When one is destroyed"):
- element-native (Elemental Knight): dissolves, leaves NOTHING.
- affinity-overlay (Neferati): dies NORMALLY, three strands take their roads.
- element-bound (Hourbound): dies AMBIGUOUSLY, compromised strand-set, death-roads run at the wrong rate or out of sequence (late/early/stalled/repeating), leaving a remnant displaced OUT OF TIME.
DISTINCTNESS (the framework's legibility requirement, plan §39-52): the Hourbound remnant is fixed in PLACE but unstuck in WHEN — the inverse of the Cinderbound, whose ember-haunt is fixed in place and time, and unlike the Air-bound (galebound) remnant, which SCATTERS on the wind. Contrast stated explicitly in body ("Set that against the other things the elements leave behind").

LOCKED HARD LIMIT honored (magic-time:17, design §3.2/§8): LOCAL RATE ONLY. The being is desynced, never a time-traveler — never history, never the Great Cycle, never undoing a death. Stated outright in "What time took" ("desynced, not unstuck from history... cannot step into a finished year or walk back a death") and re-guarded in "When one is destroyed" ("none of this is travel... does not visit the past and does not foresee"). Hooks were written to STAY inside local-rate (the "early answer" factor is a half-beat ahead of its own senses on the same exchange, NOT precognition of the future/world; the dusk-walker is a slow road, not a ghost of a future event).

CANON CONSUMED, not rewritten: three-strand death verbatim from world-systems-invariants (soul→Astral, shadow→Malstaris, spirit→Celestia-while-named; soul+shadow = life-pair). Izzus / local-rate-only / drift / Besnoumeru "eerie calm" from magic-time. Origin parallels the Cinderbound weld (collapsing time-pocket welds loose Izzus to the threads, as the World Fire welded loose Yolus) and adds the drift-of-a-lived-life route off the Besnoumeru seam.

EDGES (one only, one direction; reverse computes):
- cosmology/attunedTo → magic-time (the only navigable Time link; "time" prose-autolink behavior matches the Fire precedent on Cinderbound). Prose names Izzus/Time/Besnoumeru/Salt Tomb for readability.
- NO Besnoumeru edge: the dup-id was adjudicated in wave 5 — id 400 (the Tornia district) was renamed Corvalin and re-themed off the time axis; the name "Besnoumeru" + the Izzus seam-seat identity now live solely on id 2561 (Nektuna's second city), which carries the place-side `attunedTo→magic-time` and is magic-time's `channels` target. The Hourbound stays prose-only by the element-being convention; 2561 owns the place↔Izzus link.
- NO Salt Tomb (id 3816) edge: it is a place/ruin; per plan, prefer prose-only and let the geography thread author any place↔being link from the place side.

PROVISIONAL / FLAGGED:
- Display name "Hourbound" chosen to sit in the locked "-bound" element-bound family (Cinderbound / galebound); slug "creature-hourbound" left exactly as locked.
- Coined name "Ressa" (the slipped counter in the Besnoumeru voice inset) is a free light coinage; no Tornia/Besnoumeru personal-name sheet was consulted. Flag provisional — the inset's named speaker is itself unnamed ("a time-gang factor"), matching the magic-time:24 unnamed-broker precedent; only the off-stage colleague is named. If a name collision with an existing entity exists, the lead should retune.
- No scholar attribution used (approval-gated); all assertions stated directly per scholar-attribution.md.
