...
name: "Besnoumeru"
entityType: town
blurb: "Nektuna's second city, built where the Izzus time leyline surfaces. Its Oracle Merchants buy a stretched moment and out-trade any rival on the floor."
coordinates: [119, 203]
zoomLevel: 5
sources: ["all_sections_formatted/Rimihuica.md#L1017"]
relations:
  - { rel: spatial, kind: within, target: "2555" }
  - { rel: cosmology, kind: attunedTo, target: "magic-time", note: "the Izzus seam surfaces under the city; its merchants work it for a trading edge" }
---
...
<!-- author-notes -->
Izzus hard limit honored throughout: local rate only, never history, never the Great Cycle (per magic-time.md / world-systems-invariants). The Oracle Merchants are explicitly NOT precognitive — the "oracle" name is in-world marketing; they buy a head start, not foresight.

Mystery-with-answer (GM lever, withheld from players): the seam will NOT exhaust from ordinary trading use. Per the leyline-physics model, seams anchor to the deep primordial structure and are stable on human timescales; they only shift or rupture under catastrophic over-channeling or a great cataclysm, not from merchants stretching small pockets. The syndicates' "bottomless" claim is therefore true in practice, but they assert it because it protects lease prices, not because they have verified it. A paragon over-channeling the Besnoumeru seam could in principle rupture it (the §5 cataclysm lever) — no such event has occurred and none is authored; left as a latent hook, not a fact.

Terminology reconciliation (resolved this wave): "Oracle Merchants" (from the original stub) and "time-gangs" (from magic-time.md / creature-hourbound.md) are the same culture. Oracle Merchants = the merchant houses generally; the time-gangs = the syndicates that hold and lease seam access. The name "Besnoumeru" + the Izzus identity now consolidate on THIS entity (id 2561); the former Tornia "Besnoumeru" (id 400) was renamed Corvalin and re-themed off the time axis this wave.

Edge: cosmology/attunedTo → magic-time authored on this place (the most-specific place-side owner of the Izzus-Besnoumeru reference). magic-time.md carries the complementary channels→2561 (different kind, one direction each; not a both-ends violation). No reverse edge authored on magic-time.
