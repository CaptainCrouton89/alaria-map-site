---
id: "621"
name: "Cheykiov"
entityType: ruins
blurb: "Fortress ruins in the hills southwest of Spinefir Forest."
coordinates: [196, 228]
zoomLevel: 5
sources: ["all_sections_formatted/Clueanda.md#L1423"]
relations:
  - { rel: spatial, kind: within, target: "607" }
review:
  aiWritten: true
  archetype: ai-ok
  action: keep
  notes: "Fortress-ruins betrayal lore, on-theme, matches ruins type + central placement (196,228) 'SW of Spinefir'—consistent. Keep."
---
Fortress ruins in the hills southwest of Spinefir Forest.

Cheykiov was supposed to hold. Thick walls, strong garrison, excellent position. It fell anyway, to betrayal rather than Silzar assault. Someone opened the gates. Someone manipulated the defenders. The details are disputed; every faction blames every other faction.

Now it's ruins. The Silzar stripped what they wanted and moved on. The Gyv haven't rebuilt because no one can agree who should control it, and no one trusts anyone else's garrison.

Some say treasure lies buried in the collapse. Others say something worse than treasure was sealed away when the fortress fell. The ruins are avoided, but not forgotten.
