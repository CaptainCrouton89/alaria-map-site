---
id: "478"
name: "Rathscar"
entityType: city
blurb: "The capital city, seat of the High Camarik Council."
coordinates: [305, 158]
zoomLevel: 4
sources: ["all_sections_formatted/Clueanda.md#L549"]
relations:
  - { rel: spatial, kind: within, target: "477" }
  - { rel: polity, kind: capitalOf, target: "477" }
  - { rel: culture, kind: inhabitedBy, target: "race-human", note: "Human kingdoms; royal governance (King Salomor, Prince Edric, Queen Alisandra) and named human political structures described" }
review:
  aiWritten: true
  archetype: ai-ok
  action: keep
  notes: "Specific and consistent with Camaran's mage-licensing regime; no proximity claims."
---
The capital city, seat of the High Camarik Council. A city of orderly streets, imposing civic buildings, and endless paperwork. This is where mages must register for licenses if they wish to practice elemental magic legally. The bureaucracy is legendary—inspectors, forms, annual renewals, restricted zones.

Manufacturing district produces goods shipped across the Middle Sea. The wealthy live in townhouses near the council halls; workers crowd tenements near the factories.
