---
id: "692"
name: "Morgnor's Cradle"
entityType: region
blurb: "The domain of Morgnor, an ancient earth-attuned dragon who rules over Etherweaver goblins in the Wurmspine Mountains northwest of Thespia."
coordinates: [192, 150]
zoomLevel: 4
tags: ["state", "nation"]
sources: ["all_sections_formatted/Clueanda.md#L1807"]
relations:
  - { rel: spatial, kind: borders, target: "816", note: "SE" }
  - { rel: spatial, kind: borders, target: "1005", note: "W" }
  - { rel: spatial, kind: within, target: "388" }
review:
  aiWritten: false
  archetype: ai-ok
  action: keep
  notes: "Coherent, on-theme dragon-domain region. Map confirms mountain territory NW of Thespia. Border claims mostly match: Thespia SE/E (correct), Deadwalk W/NW (correct). Glorin SE per prose but map labels GLORIN to the S; Glorin has no entity (missing). Goblin settlement names (Lagunas/Shegaat/Dir/Addis/Norwun/Scuilli/Telleni/Goldir) are map labels with no pins (missing entities)."
---
The domain of Morgnor, an ancient earth-attuned dragon who rules over Etherweaver goblins in the Wurmspine Mountains northwest of Thespia. The goblins mine precious metals and enchant them for the dragon's ever-growing hoard.

Very rich. Very greedy. Very dangerous.

#### Morgnor's Cradle

The central mountain region where the dragon makes his home. The peaks here are rich with precious metals, and Morgnor can sense every grain of gold in his domain.

#### Goblin Settlements

Mining and crafting towns scattered through the mountains: Lagunas, Shegaat, Dir, Addis, Norwun, Scuilli, Telleni, Goldir. Each serves the dragon, mining or enchanting as their skills dictate.

#### The Etherweaver Crafters

The goblins here are Etherweavers—born with attunements to time, void, force, light, or dark. They use these abilities to enchant the metals they mine, creating items of considerable power. All of it goes to Morgnor's hoard.

#### Borders

- **East**: Thespia (formerly paid tribute; Queen Akta has stopped)
- **Southeast**: Glorin (hiding massive gold wealth; Morgnor has noticed)
- **Northwest**: Deadwalk (wasteland)
- **North**: Mountains continuing toward the coast
